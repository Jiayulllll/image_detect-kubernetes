...
def do_prediction(image, net, LABELS):
    ...
    # create a result list to store the objects
    results = []
    ...
    if len(idxs) > 0:
        ...
        for i in idxs.flatten():
            ...
            #                                                                                 boxes[i][2],
            #                                                                                 boxes[i][3]))
            # store the result in a json form instead of print it
            objects = {
...
def transfer_base64(str):
    # transfer from base64 to buffer
    transfered_str = np.fromstring(base64.b64decode(str), np.uint8)
    return transfered_str


def detect_image(base64_str):
    # convert into image format from buffer and read in color
    img = cv2.imdecode(transfer_base64(base64_str), cv2.IMREAD_COLOR)
    npimg = np.array(img)
    image = npimg.copy()
    image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
    # load the neural net.  Should be local to this method as its multi-threaded endpoint
